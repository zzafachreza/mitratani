...
export default function Loyalty({navigation, route}) {
  ...
  const getLaporan = async () => {
    ...
    const grouped = tmp
      .filter(i => i.id_petani.substr(0, 2) !== 'NM')
      .reduce((acc, curr) => {
        ...
        acc[nama].totalTimbangan += parseFloat(curr.timbangan ?? 0);
        acc[nama].totalPoin = parseFloat(
          petani.filter(i => i.id == curr.id_petani)[0].poin ?? 0,
        );
    ...
    const res_data = Object.values(grouped).sort(
      (a, b) => b.totalTimbangan - a.totalTimbangan,
    );
    console.log(res_data);
    if (key.length > 0) {
      const keyword = key.toLowerCase();

      const filtered = res_data.filter(
        i =>
  ...
  return (
    <View style={{flex: 1, backgroundColor: colors.white}}>
      <MyHeader
        title={key.length > 0 ? 'Petani / Member' : 'Ranking Loyalty'}
      />

      <View style={{padding: 20, flex: 1}}>
        {key.length == 0 && (
          <MyInput
            onChangeText={x => filterPetani(x)}
            nolabel
