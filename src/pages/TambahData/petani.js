...
export default function DataPetani({navigation, route}) {
  ...
  const [editAlamat, setEditAlamt] = useState('');
  ...
  const getPetani = async () => {
    getData('petani').then(res => {
      ...
      setTMP(tmp);
      if (route.params?.key) {
        console.log('ada filter', route.params.key);
        filterData(tmp, route.params.key);
      }
    });
  };
  ...
  const hapusData = index => {
    Alert.alert('Konfirmasi', 'Yakin ingin menghapus data petani ini?', [
  ...
  const filterData = (data, searchTerm) => {
    ...
    const filtered = data.filter(i => {
      ...
      const telepon = i.telepon.toLowerCase();
      const search = searchTerm.toLowerCase();
      return (
        nama.includes(search) || id.includes(search) || telepon.includes(search)
      );
    });
  ...
  return (
    <View style={{flex: 1, backgroundColor: 'white'}}>
              ...
              <View
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    setEditID(item.id);
                    setEditTelepon(item.telepon);
                    setEditAlamt(item.alamat);
                    setPoin(item.poin);
      ...
      <View
        ...
        <MyButton
          title="Tambah Petani"
          onPress={() => navigation.navigate('TambahPetani')}
        />
      </View>
      {/* Modal edit */}
